...
object DataFrameFromCSVFile {
  ...
  def main(args:Array[String]):Unit= {
    ...
    //spark read csv file
    val df = spark.read.csv("src/main/scala/data/csv_files/zipcodes.csv")
    df.show()
    df.printSchema()

    //read csv with options
    val df2 = spark.read.options(Map("inferSchema"->"true","delimiter"->",","header"->"true")).csv("src/main/scala/data/csv_files/zipcodes.csv")
    df2.show()
    df2.printSchema()
    ...
    val df_with_schema = spark.read.format("csv")
      .option("header", "true")
      .schema(schema)
      .load("src/main/scala/data/csv_files/zipcodes.csv")
